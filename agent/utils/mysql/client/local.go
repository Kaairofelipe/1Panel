...
func (r *Local) CreateUser(info CreateInfo, withDeleteDB bool) error {
	...
	if strings.Contains(info.Permission, ",") {
		...
		for _, ip := range ips {
			if len(ip) != 0 {
				userlist = append(userlist, fmt.Sprintf("'%s'@'%s'", EscapeString(info.Username), EscapeString(ip)))
			}
		}
	} else {
		userlist = append(userlist, fmt.Sprintf("'%s'@'%s'", EscapeString(info.Username), EscapeString(info.Permission)))
	}

	for _, user := range userlist {
		if err := r.ExecSQL(fmt.Sprintf("create user %s identified by '%s';", user, EscapeString(info.Password)), info.Timeout); err != nil {
		...
		if strings.HasPrefix(info.Version, "5.7") || strings.HasPrefix(info.Version, "5.6") {
			grantStr = fmt.Sprintf("%s identified by '%s' with grant option;", grantStr, EscapeString(info.Password))
		} else {
...
func (r *Local) ChangePassword(info PasswordChangeInfo) error {
	if info.Username != "root" {
		...
		for _, user := range userlist {
			passwordChangeSql := fmt.Sprintf("set password for %s = password('%s')", user, EscapeString(info.Password))
			if !strings.HasPrefix(info.Version, "5.7") && !strings.HasPrefix(info.Version, "5.6") {
				passwordChangeSql = fmt.Sprintf("ALTER USER %s IDENTIFIED BY '%s';", user, EscapeString(info.Password))
			}
	...
	for _, host := range hosts {
		if host == "%" || host == "localhost" {
			passwordRootChangeCMD := fmt.Sprintf("set password for 'root'@'%s' = password('%s')", EscapeString(host), EscapeString(info.Password))
			if !strings.HasPrefix(info.Version, "5.7") && !strings.HasPrefix(info.Version, "5.6") {
				passwordRootChangeCMD = fmt.Sprintf("alter user 'root'@'%s' identified by '%s';", EscapeString(host), EscapeString(info.Password))
			}
...
func (r *Local) SyncDB(version string) ([]SyncDBInfo, error) {
	...
	for _, line := range lines {
		...
		userLines, err := r.ExecSQLForRows(fmt.Sprintf("select user,host from mysql.db where db = '%s'", EscapeString(parts[0])), 300)
		if err != nil {
