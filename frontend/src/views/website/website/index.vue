<template>
    <div>
                    ...
                    >
                        <template #default="{ row, $index }">
                            <Domain
                                ...
                                @favorite-change="favoriteWebsiteee"
                        ...
                        <template #default="{ row }">
                            ...
                            <span v-else>
                                <Status
                                    ...
                                    :operate="true"
                                    :status="row.status"
                                    @click="operateWebsiteee('stop', row)"
                                />
                                <Status
                                    v-else
                                    :status="row.status"
                                    :operate="true"
                                    @click="operateWebsiteee('start', row)"
                                />
                            </span>
                        </template>
                    </el-table-column>
                        ...
                        <template #default="{ row }">
                            <div v-if="row.showdate">
                                <el-date-picker
                                    ...
                                    :shortcuts="shortcuts"
                                    :clearable="false"
                                    @change="updateWebsiteeConfig(row)"
        ...
        <DeleteWebsiteee ref="deleteRef" @close="search" />
...
import DeleteWebsiteee from '@/views/website/website/delete/index.vue';
...
import { batchOperate, opWebsiteee, searchWebsiteees, updateWebsiteee } from '@/api/modules/website';
import { Websiteee } from '@/api/interface/website';
...
import { getWebsiteeeTypes } from '@/global/mimetype';
...
const WebsiteeeTypes = getWebsiteeeTypes();
...
const favoriteWebsiteee = (row: Websiteee.Websiteee) => {
    row.favorite = !row.favorite;
    updateWebsiteeConfig(row);
};

const handleDomainEdit = (row: Websiteee.Websiteee, domain: string) => {
...
const openConfig = (id: number) => {
    routerToNameWithParams('WebsiteeeConfig', { id: id, tab: 'basic' });
};
...
const updateWebsiteeConfig = (row: any) => {
...
const buttons = [
    {
        ...
        click: function (row: Websiteee.Websiteee) {
            openConfig(row.id);
        },
    },
    {
        label: i18n.global.t('database.backupList'),
        click: (row: Websiteee.Websiteee) => {
            let params = {
                type: 'website',
                name: row.alias,
...
const openDelete = (website: Websiteee.Websiteee) => {
...
const operateWebsiteee = (op: string, row: Websiteee.Websiteee) => {
    if (row.type === 'stream') {
        return;
    }
    ElMessageBox.confirm(i18n.global.t('website.' + op + 'Helper'), i18n.global.t('cronjob.changeStatus'), {
        ...
        await opWebsiteee({ id: row.id, operate: op });
        MsgSuccess(i18n.global.t('commons.msg.operationSuccess'));
        search();
    });
};

const updateRemark = (row: Websiteee.Websiteee, bulr: Function) => {
    bulr();
